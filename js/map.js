    ...
    commissions.forEach(comm => {
      if (comm.lat && comm.lng) {
        const marker = L.marker([comm.lat, comm.lng], {
          icon: this.createCustomIcon(comm.status, Number(comm.rewardPhp) > 0 ? `₱${Number(comm.rewardPhp).toFixed(0)}` : 'TBA')
        }).addTo(map);
        ...
        const popupContent = `
          <div style="font-family: 'Plus Jakarta Sans', sans-serif; color: #0f172a; padding: 4px; min-width: 220px;">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px;">
              ...
              <span style="font-weight: 800; color: #b45309; font-family: monospace; font-size: 13px;">${Number(comm.rewardPhp) > 0 ? `₱${Number(comm.rewardPhp).toFixed(0)}` : 'TBA'}</span>
            </div>
